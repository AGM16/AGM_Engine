			...
			if (current_node->Contains(center_position))
			{
				if (current_node->children.empty())
				{
					...
					else
					{
						...
						if (Insert(current_node->objects, current_node->rect.centre) != false)
						{
							//Insert new object
							ret = Insert(GO, center_position);
						    break;	
						}
						else
						{
	...
	while (!queue_qnode.empty())
	{
		...
		if (current_node->Contains(center_position))
		{
			...
			if (current_node->objects == GO && current_node->rect.centre.Equals(center_position))
			{
				...
				ret = true;
				break;
				//Check if the brothers of this node have objects inside
			}
		}
...
{
	bool ret = false;
	Component_Transformation* transform = (Component_Transformation*)GO->Get_Component(MESH);
	Component_Mesh* mesh = (Component_Mesh*)GO->Get_Component(MESH);
	if (mesh != nullptr)
	{
		...
		{
			ret = root->Insert(GO, center_point_go);
		}
	}
...
bool p2QuadTree::Remove(GameObject* GO)
{
	bool ret = false;
	Component_Mesh* mesh = (Component_Mesh*)GO->Get_Component(MESH);
	float2 center_point_go = float2(mesh->Get_OBB_Bounding_Box().CenterPoint().x, mesh->Get_OBB_Bounding_Box().CenterPoint().z);
	if (root != nullptr && root->Contains(center_point_go))
	{
